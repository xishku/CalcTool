...
class KLineViewerOptimized:
    ...
    def create_control_panel(self, parent):
        ...
        tk.Label(left_frame, text=f"股票代码: {self.display_stock_code}", 
                bg="#f0f0f0", font=('Arial', 10, 'bold')).pack(
                side=tk.LEFT, padx=(0, 20))
        
        # 日期范围
        date_range = f"日期范围: {self.start_date} 到 {self.end_date}"
        tk.Label(left_frame, text=date_range, bg="#f0f0f0").pack(
                side=tk.LEFT, padx=(0, 20))
        
        # 定位按钮
        locate_btn = tk.Button(left_frame, text="定位到日期", 
                              command=self._locate_to_date,
                              bg="#4a6fa5", fg="white", padx=10)
        locate_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        # 日期显示标签
        self.date_label = tk.Label(left_frame, text="", bg="#f0f0f0", 
                                  fg="#d9534f", font=('Arial', 10, 'bold'))
    ...
    def _show_annotation_for_date(self, date_str, x_pos, y_pos, 
                                 ax, is_cursor=False):
        """为指定日期显示注释"""
        ...
        try:
            ...
            self.current_annotation = ax.text(
                x_pos + x_offset, y_pos + y_offset, text,
    ...
    def _on_add(self, sel):
        ...
        try:
            ...
            for date_str, patch in self.date_to_patch_map.items():
                if patch == artist:
                    # 只处理蜡烛矩形，忽略均线
                    if not (date_str.endswith('_MA5') or 
                           date_str.endswith('_MA10') or
                           date_str.endswith('_MA20') or
                           date_str.endswith('_MA30') or
                           date_str.endswith('_MA60')):
            ...
            if target_date_str is not None:
                ...
                # 获取注释文本
                text = self._show_annotation_for_date(
                    target_date_str, x_pos, y_pos, self.ax1, is_cursor=True)
                if text:
                    sel.annotation.set_text(text)
                    sel.annotation.get_bbox_patch().set(
                        alpha=0.9, facecolor='lightyellow',
        ...
        if len(self.current_display_df) > 0:
            ...
            text = (f"显示 {len(self.current_display_df)} 个交易日\n"
                   f"日期范围: {date_str} 到 {last_date}")
            sel.annotation.set_text(text)
            sel.annotation.get_bbox_patch().set(
                alpha=0.9, facecolor='lightblue',
    ...
    def _locate_to_date(self):
        ...
        date_str = simpledialog.askstring(
            ...
            f"日期范围: {self.start_date_str} 到 {self.end_date_str}",
            parent=root)
        ...
        try:
            ...
            if date_idx is not None:
                ...
                if 0 <= highlight_idx_in_display < len(self.current_display_df):
                    date_str = str(self.current_display_df.iloc[
                        highlight_idx_in_display]['date'])
                    x_pos = highlight_idx_in_display
                    y_pos = self.current_display_df.iloc[
                        highlight_idx_in_display]['Close']
                    self._show_annotation_for_date(date_str, x_pos, y_pos, 
                                                 self.ax1)
    ...
    def _refresh_chart(self, highlight_idx=None, is_locate=False, 
                      locate_date_str="", min_diff=0):
        """刷新图表显示"""
        if not hasattr(self, '_current_display_start'):
            self._current_display_start = max(0, len(self.df) - 
                                              self.display_days)
        ...
        display_end = min(len(self.df), 
                         self.current_display_start + self.display_days)
        
        if display_end - self.current_display_start < 10:
            self.current_display_start = max(0, len(self.df) - 
                                             self.display_days)
            display_end = len(self.df)
            self._current_display_start = self.current_display_start
        
        # 获取显示数据
        self.current_display_df = self.df.iloc[
            self.current_display_start:display_end].copy()
        ...
        if is_locate:
            if min_diff > 0:
                actual_date = self.df.iloc[highlight_idx]['Date'].strftime(
                    '%Y-%m-%d')
                title = (f'{self.display_stock_code} 日K线 - 定位到: '
                        f'{actual_date} (输入: {locate_date_str}, '
                        f'相差{min_diff}天)')
            else:
                title = (f'{self.display_stock_code} 日K线 - 定位到: '
                        f'{locate_date_str}')
        ...
        for period in [5, 10, 20, 30, 60]:
            if len(self.current_display_df) >= period:
                ...
                for i in range(len(self.current_display_df)):
                    if i >= period - 1:
                        ma = self.current_display_df['Close'].iloc[
                            i-period+1:i+1].mean()
                    else:
                        ma = np.nan
                    ma_values.append(ma)
        ...
        if len(self.current_display_df) > 0:
            first_date = self.current_display_df.iloc[0]['Date'].strftime(
                '%Y-%m-%d')
            last_date = self.current_display_df.iloc[-1]['Date'].strftime(
                '%Y-%m-%d')
            ...
            if self.date_label:
                if highlight_idx is not None and 0 <= highlight_idx < len(self.df):
                    highlight_date = self.df.iloc[highlight_idx]['Date'].strftime(
                        '%Y-%m-%d')
                    self.date_label.config(
                        text=f"当前显示日期: {highlight_date}")
        
        # 添加高亮标记
        highlight_idx_in_display = None
        if highlight_idx is not None and \
           self.current_display_start <= highlight_idx < display_end:
            highlight_idx_in_display = highlight_idx - self.current_display_start
            if 0 <= highlight_idx_in_display < len(self.current_display_df):
                loc_date = self.current_display_df.iloc[
                    highlight_idx_in_display]['Date']
                loc_close = self.current_display_df.iloc[
                    highlight_idx_in_display]['Close']
    ...
    def _move_days(self, days):
        ...
        if self.current_highlight_idx is None:
            self.current_highlight_idx = (self._current_display_start + 
                                          self.display_days // 2)
    ...
    def _find_date_index(self, target_date_str):
        ...
        try:
            ...
            if date_idx is not None:
                if min_diff > 0:
                    print(f"提示: 输入的日期 {target_date_str} 没有对应数据，"
                         f"定位到最接近的日期 "
                         f"{self.df.iloc[date_idx]['Date'].strftime('%Y-%m-%d')} "
                         f"(相差{min_diff}天)")
                else:
    ...
    def _process_dataframe(self):
        ...
        # 根据实际列名处理数据
        if ('r_open' in self.df.columns and 'r_high' in self.df.columns and
            'r_low' in self.df.columns and 'r_close' in self.df.columns):
            self.df['Open'] = self.df['r_open']
            self.df['High'] = self.df['r_high']
            self.df['Low'] = self.df['r_low']
            self.df['Close'] = self.df['r_close']
            print("使用复权价格数据")
        elif ('open' in self.df.columns and 'high' in self.df.columns and
              'low' in self.df.columns and 'close' in self.df.columns):
    ...
    def _process_price_columns(self):
        """处理价格列"""
        # 确保价格是数值类型
    ...
    def _process_volume_column(self):
        """处理成交量列"""
    ...
    def load_data(self):
        ...
        print(f"正在加载 {original_code} (标准化: {normalized_code}) 的数据，"
        ...
        try:
            ...
            for code_variant in code_variants:
                try:
                    ...
                    self.df = agent.read_kdata_cache(
                        code_variant, self.start_date, self.end_date)
                    
                    if self.df is not None and not self.df.empty:
                        print(f"✓ 代码 {code_variant} 成功获取 "
                             f"{len(self.df)} 条数据")
                        self.stock_code = code_variant
                        self.display_stock_code = f"{original_code} ({code_variant})"
                        break
                    else:
            ...
            # 处理价格列
            if not self._process_price_columns():
                return False
            
            # 处理成交量列
            if not self._process_volume_column():
                return False
            
            # 处理日期列
            if not self._process_date_column():
                return False
    ...
    def show(self):
        ...
        if self.target_date:
            ...
            if initial_highlight_idx is not None:
                print(f"图表将默认定位到: "
                     f"{self.df.iloc[initial_highlight_idx]['Date'].strftime('%Y-%m-%d')}")
            else:
                print(f"警告: 无法定位到日期 {self.target_date}，"
                     f"将显示最近的数据")
        ...
        self._refresh_chart(highlight_idx=initial_highlight_idx, 
                          is_locate=is_locate, 
                          locate_date_str=self.target_date or "", 
                          min_diff=0)
...
def main():
    ...
    parser.add_argument('--stock', type=str, default='601398', 
                       help='股票代码')
    parser.add_argument('--start', type=str, default='20260101', 
                       help='开始日期 (YYYYMMDD)')
    parser.add_argument('--end', type=str, default='20260401', 
                       help='结束日期 (YYYYMMDD)')
    parser.add_argument('--target', type=str, 
                       help='默认定位到的目标日期 (YYYYMMDD 或 YYYY-MM-DD)')
