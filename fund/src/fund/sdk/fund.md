...
├── fund_daily_nav.py        # 基金每日净值（全量历史 + SQLite 存储）
...
## 6. fund_daily_nav.py — 基金每日净值

### 类: `FundDailyNAV`

从同花顺爱基金获取每只基金的**全量历史每日净值**，存入 SQLite 数据库，支持批量爬取和按日期查询。

### 数据源

| 数据源 | URL | 说明 |
|--------|-----|------|
| 单位净值 | `https://fund.10jqka.com.cn/{code}/json/jsondwjz.json` | **全量**历史单位净值（JSONP） |
| 累计净值 | `https://fund.10jqka.com.cn/{code}/json/jsonljjz.json` | **全量**历史累计净值（JSONP） |

> 与 `get_fund_10jqka_info()` 的区别：后者只保留最新值和最近 20 条走势，本模块**保留全部历史数据**并持久化到 SQLite。
...
CREATE TABLE fund_daily_nav (
    ...
    unit_nav       REAL,             -- 单位净值
    cumulative_nav REAL,             -- 累计净值（部分日期可能为 NULL）
    PRIMARY KEY (fund_code, nav_date)
);

-- 元信息表
CREATE TABLE fund_nav_meta (
    fund_code    TEXT PRIMARY KEY,
    fund_name    TEXT,
    first_date   TEXT,               -- 最早净值日期
    last_date    TEXT,               -- 最新净值日期
...
| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| `fetch_and_store_one(code)` | `code: str` | `bool` | 🧪 **单只测试**：获取一只基金的完整历史净值并存入数据库 |
| `fetch_and_store_all(max_workers)` | `max_workers: int=3` | `dict` | 🚀 **全量批量**：获取全部基金的历史净值（并发爬取） |
| `query_nav(code, start_date, end_date)` | `code: str`, `start_date: str=None`, `end_date: str=None` | `List[Dict]` | 查询指定基金在日期区间的净值数据 |
| `query_meta(code)` | `code: str=None` | `List[Dict]` | 查询基金元信息（None 返回全部概要） |
| `stats()` | — | `dict` | 统计概览（总记录数、基金数、最新日期） |
| `store_nav_data(code, nav_data, name)` | `code: str`, `nav_data: list`, `name: str=""` | `int` | 将净值数据写入数据库（事务批量插入，幂等） |
...
fdn = FundDailyNAV(db_path="fund_nav.db")

# 测试一只基金
fdn.fetch_and_store_one("001438")  # 易方达瑞享混合E

# 查询
nav = fdn.query_nav("001438", start_date="2025-01-01")
for r in nav[:5]:
    print(f"{r['nav_date']}  单位净值: {r['unit_nav']}")
```
...
```python
fdn = FundDailyNAV()
result = fdn.fetch_and_store_all(max_workers=3)
...
```bash
# 测试单只基金
python fund_daily_nav.py --test 001438

# 批量全量爬取
python fund_daily_nav.py --all --workers 5

# 查看统计
python fund_daily_nav.py --stats

# 查询某基金净值
python fund_daily_nav.py --query 001438

# 指定数据库路径
python fund_daily_nav.py --db ../data/fund_nav.db --test 001438
```
...
```
                    ...
                    ┌─────────────────┼─────────────────┐
                    ▼                 ▼                  ▼
          _get_10jqka_session()  _TENJQKA_SEM   _bump_10jqka_fail()
          (curl_cffi Session)   (全局限流2并发)   (连续失败自动刷新)
                    │
                    ▼
          ┌─────────────────────────┐
          │  jsondwjz.json           │
...
| 阶段 | 内容 | 命令 |
|------|------|------|
| 1 | 单只基金测试 | `--test 001438` |
| 2 | 小批量验证（~10 只） | 手动循环 |
| 3 | 全量爬取 | `--all --workers 5` |
| 4 | 定期更新（增量） | 定时任务调用 `fetch_and_store_all()` |

### 注意事项

- **限流严格**：10jqka 反爬严格，默认 2 并发 + 0.3-0.8s 间隔随机延时
- **幂等写入**：每次 `fetch_and_store_one` 会先删除旧数据再插入，确保重复运行不会重复
- **累计净值匹配**：单位净值和累计净值按日期匹配，若某日无累计净值则该字段为 NULL
- **依赖**：复用 `fund_list.py` 的 curl_cffi Session、UA 池、全局限流信号量
- **存储依赖**：使用项目 `dbutil` 模块的 `Connection` 进行 SQLite 操作
...
requests >= 2.28
beautifulsoup4 >= 4.11
curl_cffi >= 0.6              # 10jqka 反爬（可选，没有则回退标准 requests）
```
...
```
link_data_fetcher.py          parse_text.py            fund_daily_nav.py
  (通用 HTTP 工具)              (HTML 解析示例)          (每日净值 + SQLite)
        │                                                   │
        ▼                                                   │
  fund_list.py ◄──────────── fund_stock_list.py ◄───────────┘
  (基金列表 + 详细信息)          (基金持仓明细)         (复用 Session / 限流)
        │                           │
        ...
        │  ├─ jjfl (费率页)
        │  │ (认购/申购/赎回费率)
        │  ├─ fund.10jqka.com.cn/{code}/json/jsondwjz.json  ← fund_daily_nav
        │  │ (全量历史单位净值 → SQLite fund_nav.db)
        │  └─ fundcode_search
        │     (全量基金列表)
        │
        ▼
