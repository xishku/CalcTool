...
class FundStockListTool:

    def __init__(self):

        self.fund_list = FundList()

    def get_fund_list(self):
        yield from self.fund_list.get_fund_structured_list()

    def get_all_fund_codes(self):
        for fund_code in self.get_fund_list():
            yield fund_code['code']

    def get_fund_holdings_by_year(self, fund_code: str, year, topline: int = 100) -> List[Dict[str, Any]]:
        """
        获取基金持仓信息
    ...
    def get_fund_stock_list_to_file(self, fund_codes, file_path: str):
        with open(file_path, 'w', encoding='utf-8') as f:
            for fund_code in fund_codes:
                ...
                for year in range(datetime.now().year, 2015, -1):
                    try:
                        # 获取简单持仓信息
                        holdings = self.get_fund_holdings_by_year(fund_code, year, 100)
...
def main():
    ...
    # fund_codes = tool.get_all_fund_codes()
    fund_codes = ['011665', "011399", "013552", "007355", "014193", "009550", "012820", "009715", "000697",
                            "470009",
                            "011826",
                            "011418",
                            "501206",
    ...
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fund_stock_list.txt")
    print(file_path)
    tool.get_fund_stock_list_to_file(fund_codes, file_path=file_path)
